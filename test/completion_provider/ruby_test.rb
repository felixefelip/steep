...
class CompletionProvider__RubyTest < Minitest::Test
  ...
  def test_on_lower_identifier
    with_checker do
      CompletionProvider::Ruby.new(source_text: <<-EOR, path: Pathname("foo.rb"), subtyping: checker, literal_method_registry: Steep::Project::LiteralMethodRegistry.new, delegation_registry: Steep::Project::DelegationRegistry.new).tap do |provider|
req
...
a, b = []
  ...
  def test_simple_method_name_item_two_defs
    with_checker <<~RBS do
      ...
      RBS
      CompletionProvider::Ruby.new(source_text: <<~RUBY, path: Pathname("foo.rb"), subtyping: checker, literal_method_registry: Steep::Project::LiteralMethodRegistry.new, delegation_registry: Steep::Project::DelegationRegistry.new).tap do |provider|
          TestClass.new.f
        RUBY
